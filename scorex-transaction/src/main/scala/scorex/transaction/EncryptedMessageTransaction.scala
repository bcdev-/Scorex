...
case class EncryptedMessageTransaction(timestamp: Long,
  ...
  val FeeMultiplier = ((rawMessage.length - EncryptedMessageTransaction.VanityHeaderSize) / 64) + 1
  ...
  override lazy val json: JsObject = Json.obj(
    ...
    "raw_message" -> Base64.encode(rawMessage),
    "size" -> bytes.length,
  ...
  lazy val messageHashValid = java.util.Arrays.equals(Blake2b256.hash(Bytes.concat(nonce, rawMessage)), rawMessageHash)
...
object EncryptedMessageTransaction extends Deser[EncryptedMessageTransaction] {
  ...
  def parseTail(bytes: Array[Byte]): Try[EncryptedMessageTransaction] = Try {
    ...
    val rawMessageHash = bytes.slice(s1 + 16, s1 + 16 + Blake2b256.DigestSize)
    val s2 = s1 + 16 + Blake2b256.DigestSize
  ...
  def create(timestamp: Long,
    ...
    // rawMessageHash makes it possible to prune older messages in the future
    lazy val rawMessageHash = Blake2b256.hash(Bytes.concat(nonce, rawMessage))
  ...
  private def encryptMessage(timestamp: Long,
    ...
    val rawMessageHash = Blake2b256.hash(Bytes.concat(nonce, rawMessage))
